...
def custom_wait_clickable_and_click(selector, action='click', action_param=None, attempts=5):
    ...
    while count < attempts:
        try:
            ...
            elem = WebDriverWait(browser, 10).until(ec.element_to_be_clickable((By.XPATH, selector)))
            if action == 'click':
                elem.click()
            else:
...
with open("output.txt", 'w', encoding="utf-8") as f:
    ...
    for filename in os.listdir(directory):
        ...
        ui.WebDriverWait(browser, 10).until(ec.element_to_be_clickable((by.By.XPATH, '//*[.="Log in"]')))
        browser.find_element(by='xpath', value='//*[.="Log in"]').click()

        login = "gromnekot@gmail.com"
        password = "X6$AwCo%.omu3Z^"

        ui.WebDriverWait(browser, 10).until(ec.element_to_be_clickable((by.By.NAME, 'email')))
